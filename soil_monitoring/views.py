...
class SensorReadingViewSet(
    ...
    def latest(self, request):
        ...
        latest_id_subq = (
            SensorReading.objects.filter(field_id=OuterRef("pk"))
            .order_by("-ts")
            .values("id")[:1]
        )
        ...
        latest_readings = (
            SensorReading.objects.filter(id__in=list(latest_ids))
...
class RecommendationViewSet(
    ...
    def deactivate_all(self, request):
        ...
        if not field_id:
            return Response(
                {"detail": "field_id is required"}, status=status.HTTP_400_BAD_REQUEST
            )

        field = get_object_or_404(Field, id=field_id, farm__owner=request.user)
        updated = Recommendation.objects.filter(field=field, is_active=True).update(
            is_active=False
        )
...
class FieldAnalyticsViewSet(viewsets.ViewSet):
    ...
    def statistics(self, request):
        ...
        if not readings.exists():
            return Response(
                {"detail": "No readings available for this period"},
        ...
        stats["moisture_percent_avg"] = (
            stats["moisture_vwc_avg"] * 100
            if stats["moisture_vwc_avg"] is not None
            else None
        )
    ...
    def dashboard(self, request):
        ...
        fields_with_sensors = (
            SensorReading.objects.filter(field__in=owner_fields)
            .values("field")
            .distinct()
            .count()
        )
        ...
        readings_today = SensorReading.objects.filter(
            field__in=owner_fields, ts__gte=today_start
        ).count()
        ...
        recent_recommendations = active_recs.select_related("field").order_by(
            "-created_at"
        )
        if connection.features.has_native_duration_field:
        ...
        return Response(
            {
                ...
                "recent_recommendations": RecommendationSerializer(
    ...
    def analyze(self, request):
        ...
        latest_reading = (
            SensorReading.objects.filter(field=field).order_by("-ts").first()
        )
        if not latest_reading:
            return Response(
                {"detail": "No readings available for analysis"},
                status=status.HTTP_404_NOT_FOUND,
            )
